#### COMMUNITY HYPERVOLUME GENERATION AND PAIRWISE COMPARISONS ####
...
# Community high vs low analysis

#create empty dataframes for analysis output
...
## adding HV pairwise stats to dataframes - complete stats dataframe for all possible combinations of caste/highlow
for (i in 1:4) {
  for (j in 1:4) {
    # pair of HVs
...
legend3d("topleft",legend=c('Above','Below'),pch=16,col=colourpalette,cex=1.3)
...
#split by caste, year
yearcomsplit<-z.hvbees%>%select(pca,wing_resid,head_resid,Caste_Assigned_CT,Year)
yearcomsplit=split(yearcomsplit,paste(yearcomsplit$Caste_Assigned_CT,yearcomsplit$Year,sep=""))

#create hypervolumes from split data
ycHV=mapply(function(x,y)
  hypervolume_gaussian(x[,c("pca","wing_resid","head_resid")],
                       samples.per.point=100,name=y),
  x=yearcomsplit,
  y=names(yearcomsplit))

#save as total hypervolume list for year community HV
...
#for all possible combinations of year/caste
for (i in 1:8) {
  for (j in 1:8) {
    # pair of HVs
...
#convert temp to daily resolution
...
#join into new temperature dataset with all years, calculate mean and se
workerdevelopment_temp<-rbind(w18devtemp,w19devtemp,w21devtemp,w22devtemp)%>%
  group_by(Year)%>%summarise(mean=mean(AirTemperature),
                             se=(sd(AirTemperature))/(sqrt(length(AirTemperature))))

#then assign years as A B C D from lowest to highest temperature
...
#queens DEVELOPMENT period 

#take queen development period as July-September in previous year (from Ranta & Lundberg, 1981)
#filter for development period in previous year, create y2 for emergence year, calculate mean and se
qdevtemps<-temp%>%filter(doy>=182&doy<=244)%>%mutate(y2=Year+1)%>%
  filter(y2<2023&y2>2017)%>%group_by(y2)%>%
  summarise(mean=mean(AirTemperature),
               se=(sd(AirTemperature))/(sqrt(length(AirTemperature))))
...
legend3d("topright",legend=c('Queen','Worker'),pch=16,col=colourpalette,cex=1.3)

#plot workers in blue
hvplot_w(wA)
...
# note - not used in final project as year/highlow/caste not possible on species-specific level
