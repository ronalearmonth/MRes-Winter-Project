...
rm(list=ls())
#does not require "1 - Data preparation.R"
...
#get_volume creates named vector of volumes for each hv
...
#create new df including all hv volumes
...
#plot volume by vegzone for each caste
ch_volplot<-ggplot(data=highlowvol)+geom_boxplot(aes(x=Caste,y=Volume,fill=Highlow))+
  theme_classic()+ scale_fill_manual(values=c("#CC6677", "#88CCEE"))+labs(x="Caste")+
  guides(fill=guide_legend(title="Vegetation Zone"))
...
#set sample draw as minimum sample size - worker 2019 abnormally low. (~10% of other sample sizes), therefore set
#min sample size to lowest after this and acknowledge bias due to W2019
...
# add columns for mean development temperature
...
#create separate models for each caste for effects of devyear and species
