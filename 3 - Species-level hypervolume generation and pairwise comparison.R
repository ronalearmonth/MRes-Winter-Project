...
#investigate high vs low by species and caste (without year)
...
sp.bees<-z.hvbees%>%filter(Bombus_Species=="monticola"|Bombus_Species=="balteatus"|Bombus_Species=="lapponicus")


#create new temporary df
tempbees2<-sp.bees%>%select(pca,wing_resid,head_resid,Bombus_Species,Caste_Assigned_CT,highlow)

#split by species, caste, altitude
tempbees2=split(tempbees2,paste(tempbees2$Bombus_Species,
                                tempbees2$Caste_Assigned_CT,
                                tempbees2$highlow,sep=""))
...
hvplot_scaled(WH)
legend3d("topright",legend=c('Balteatus','Lapponicus','Monticola'),pch=16,col=colourstbc,cex=1)
hvplot_scaled(QL)
...
#only pratorum if sample size reduced to 20
...
sp.bees2<-z.hvbees%>%filter(Bombus_Species=="lapponicus"|Bombus_Species=="monticola"|Bombus_Species=="pratorum")

#create year/species/caste df
tempbees3<-sp.bees2%>%select(pca,wing_resid,head_resid,Bombus_Species,Caste_Assigned_CT,Year)

#split by species, caste, altitude
tempbees3=split(tempbees3,paste(tempbees3$Bombus_Species,
                                tempbees3$Caste_Assigned_CT,
                                tempbees3$Year,sep=""))
...
# PLOT SPECIES HYPERVOLUMES 

#save each caste/year combination to plot
#queens
...
#plot each hv by mean annual temperature
hvplot_scaled(Q2018)
...
legend3d("topright",legend=c('Lapponicus','Monticola','Pratorum'),pch=16,col=colourstbc,cex=1)
...
legend3d("topleft",legend=c("balteatus","jonellus","lapponicus",
                            "lucorum","monticola","pascuorum","pratorum"),pch=16,col=colourstbc,cex=1)
